...
# Путь соответствует volume в docker-compose: ../results:/results
RESULTS_DIR = "/results"
...
logging.basicConfig(
    level=logging.INFO,
    format="[SPARK-APP] %(asctime)s | %(message)s",
    datefmt="%H:%M:%S"
)
...
spark = (
    ...
    .master("local[*]")
    # spark — hostname контейнера заданный в docker-compose
    .config("spark.driver.host", "spark")
    # 0.0.0.0 — слушаем все сетевые интерфейсы внутри контейнера
    .config("spark.driver.bindAddress", "0.0.0.0")
    # namenode — hostname NameNode контейнера в docker-compose
...
def step_read():
    df = spark.read.csv(
        # namenode:9000 — адрес NameNode внутри Docker сети
        "hdfs://namenode:9000/data/input/ecommerce_orders.csv",
        header=True,
        inferSchema=True
    )
    count = df.count()
